...
import { fetchUserStats } from '../../services/results.service';
  ...
  return (
    <div className="min-h-screen bg-bg-primary text-text-primary flex flex-col">
      ...
      <main className="flex-1 px-4 py-8 max-w-2xl mx-auto w-full flex flex-col gap-5">
        ...
        {/* 🆕 Part 5 — return-to-lobby vote (Solid Purple Button) */}
        <div className="flex items-center justify-between gap-4 pt-2 mt-2 border-t border-bg-primary/20">
          <div className="flex items-center gap-3 flex-wrap">
            <button
              className="px-4 py-2 cursor-pointer rounded-lg font-semibold transition-all shadow-sm bg-accent-primary/80 text-white hover:brightness-110"
              onClick={() => voteReturnToLobby(!myVote)}
            >
              {myVote ? '✔ Waiting for others…' : 'Return to Lobby'}
            </button>
            <span className="text-sm text-text-muted font-medium bg-bg-primary/30 px-3 py-1 rounded-full">
              {votes.size}/{activePlayers.length} votes
            </span>
          </div>
          <button
            className="flex items-center gap-1.5 cursor-pointer px-4 py-2 rounded-lg border border-bg-primary/20 text-text-muted hover:text-red-400 hover:border-red-400/30 transition-colors"
